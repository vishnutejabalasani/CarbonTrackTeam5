  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Milestones & Carbon Objectives</h1>
          <p className="text-slate-500 text-sm mt-1">Set carbon limits, analyze operational targets, and earn corporate ESG badges.</p>
        </div>
        <button
      ...
      <div className="grid lg:grid-cols-[1fr_380px] gap-8">
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white rounded-2xl border border-slate-200/60 p-6 shadow-sm space-y-6">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h2 className="font-bold text-slate-950 text-sm flex items-center gap-2">
                <Target className="text-brand-850" size={16} />
                Active Reduction Target
              </h2>
              {currentGoal && (
                <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold ${
                  currentGoal.onTrack ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"
                }`}>
            ...
            {currentGoal ? (
              <div className="grid md:grid-cols-[auto_1fr] gap-6 items-center">
                ...
                <div className="relative w-32 h-32 flex items-center justify-center shrink-0 mx-auto">
                  <svg className="w-full h-full transform -rotate-95">
                    ...
                    <circle
                      ...
                      cy="64"
                      r={radius}
                      className="text-brand-800 transition-all duration-500 ease-out"
                      strokeWidth="8"
                ...
                <div className="space-y-4">
                  <div>
                    <h3 className="font-bold text-slate-950 text-base">{currentGoal.title}</h3>
                  ...
                  <div className="flex gap-4 text-xs text-slate-500">
                    <div>
                      ...
                      <p className="font-semibold text-slate-800 mt-0.5">{currentGoal.startDate}</p>
                    </div>
                    <div>
                      <p className="text-[9px] uppercase font-bold text-slate-400">Target End</p>
                      <p className="font-semibold text-slate-800 mt-0.5">{currentGoal.endDate}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="text-center py-10 space-y-4">
                <Sprout className="text-slate-200 mx-auto" size={40} />
                <div>
                  <p className="text-xs text-slate-500 font-medium">No active carbon goals.</p>
          ...
          <div className="bg-white rounded-2xl border border-slate-200/60 p-6 shadow-sm space-y-4">
            <h2 className="font-bold text-slate-900 text-sm">Target Performance Ledger</h2>
            
            {history.filter((g) => g.status !== "active").length === 0 ? (
              <p className="text-xs text-slate-450 py-6 text-center">No historical completed target logs found.</p>
            ) : (
                  ...
                  .map((g) => (
                    <div key={g.id} className="flex items-center justify-between py-3">
                      ...
                      <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-bold ${
                        g.status === "completed" ? "bg-brand-50 text-brand-850" : "bg-slate-50 text-slate-400"
                      }`}>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 p-6 shadow-sm space-y-5">
          <div>
            <h2 className="font-bold text-slate-950 text-sm flex items-center gap-2">
              <Award className="text-brand-850" size={16} />
              Corporate Badges
            </h2>
              ...
              return (
                <div
                  ...
                  className={`p-4 border rounded-2xl flex flex-col items-center justify-center text-center transition duration-300 relative group overflow-hidden ${
                    earned
                      ? "border-emerald-200 bg-gradient-to-b from-brand-50/20 to-emerald-50/10 text-brand-900 shadow-sm"
                      : "border-slate-150 bg-slate-50/20 text-slate-400 opacity-60"
                  }`}
                >
                  ...
                  <div
                    className={`w-9 h-9 rounded-xl flex items-center justify-center mb-2 shadow-sm relative z-10 transition duration-300 group-hover:scale-105 ${
                      earned ? "bg-white text-brand-800 border border-brand-100/50" : "bg-slate-100 text-slate-400"
                    }`}
                  >
                  ...
                  <p className="text-[9px] text-slate-450 mt-1 leading-tight line-clamp-2 relative z-10">
                    {badge.description}
                  </p>
                </div>
              );
